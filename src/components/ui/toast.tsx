...
const Toast = React.forwardRef<HTMLDivElement, ToastProps>(
  ({ className, variant = "default", title, description, onClose, ...props }, ref) => {
    React.useEffect(() => {
      if (props.duration && onClose) {
        const timer = setTimeout(onClose, props.duration)
      ...
      <div
        ...
        className={cn(
          "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all",
          {
            ...
            "border border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-900/20 dark:text-green-200": variant === "success",
          },
          className
        )}
        {...props}
